...
class Graph:
    ...
    def to_graphviz_enhanced(
        ...
        """
        Enhanced version of to_graphviz with additional filtering and highlighting capabilities.
        
        Args:
            colorscheme: a cmap colorscheme
            minimum_penwidth: minimum width for edge lines
            maximum_penwidth: maximum width for edge lines
            layout: graph layout type
            seed: random seed for color generation
            score_threshold: threshold value for edge filtering
            threshold_type: 'above' or 'below' to specify threshold direction
            exclude_nodes: list of node names to exclude from visualization
            highlight_nodes: list of node names to highlight in bold
        """
        g = pgv.AGraph(directed=True, bgcolor="white", overlap="false", splines="true", layout=layout)
        ...
        # Initialize sets for tracking nodes to include
        nodes_to_include = set()
        exclude_nodes = set() if exclude_nodes is None else set(exclude_nodes)
        highlight_nodes = set() if highlight_nodes is None else set(highlight_nodes)

        # First pass: determine which edges meet the score threshold
        for edge in self.edges.values():
            ...
            if score_threshold is not None:
                if threshold_type == "above":
                    include_edge = abs(score) >= score_threshold
                else:  # below
                    include_edge = abs(score) <= score_threshold
            
            if include_edge:
                nodes_to_include.add(edge.parent.name)
                nodes_to_include.add(edge.child.name)
        ...
        # Add nodes
        for node in self.nodes.values():
            if (node.in_graph and 
                node.name in nodes_to_include and 
                node.name not in exclude_nodes):
                ...
                # Apply bold fontweight for highlighted nodes
                if node.name in highlight_nodes:
                    node_attrs['fontweight'] = 'bold'
        ...
        for edge in self.edges.values():
            ...
            # Check if edge's nodes are included
            nodes_included = (
                edge.parent.name not in exclude_nodes and
                edge.child.name not in exclude_nodes and
                edge.parent.name in nodes_to_include and
                edge.child.name in nodes_to_include
            )
            
            if include_edge and nodes_included:
                normalized_score = (abs(score) - min_score) / (max_score - min_score) if max_score != min_score else abs(score)
                penwidth = max(minimum_penwidth, normalized_score * maximum_penwidth)
                g.add_edge(
                    edge.parent.name,
